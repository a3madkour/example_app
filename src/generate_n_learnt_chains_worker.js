...
onmessage = function(message) {
  const selection = message.data[0]
  var graphs = []
            var value = selection;
            const keyss = Object.keys(selection);
            var selected_graphs = [];
  console.log("everybodu");
  ...
  //       });
    ...
    var num_samples = message.data[2];
            ...
            for (var i = 0; i < graphs.length; i++) {
                if (graphs[i][keyss[0]] > value[keyss[0]][0] && graphs[i][keyss[0]] < value[keyss[0]][1]) {
                    if (graphs[i][keyss[1]] > value[keyss[1]][0] && graphs[i][keyss[1]] < value[keyss[1]][1]) {
                        selected_graphs.push(graphs[i]);
                        vec.push_back(graphs[i].chain_str)
                    }
                }
            }
