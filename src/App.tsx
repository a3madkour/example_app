...
let graphs: any;
export async function connectToDatabase(graphs: any, trained_graphs: any, selected_graphs: any, user_id: any) {
    ...
    try {
        ...
        for (let i = 0; i < selected_graphs.length; i++) {
            const result = await collGraphs.updateOne({ hash: selected_graphs[i].hash }, selected_graphs[i], { upsert: true });
        ...
        const result = collExamples.insertOne({
            user_id: user_id, selected_graphs: selected_graphs_hashes, original_graphs: original_graphs_hashes, trained_graph: trained_graphs_hashes
        })

        console.log(selected_graphs_hashes);
...
function App() {
    ...
    const [state, setState] = useState({ user_id: "", temp_user_id: "", num_samples: 10, module: temp, graphs: graphs_s, trained_graphs: trained_graphs, x: "leniency", y: "mission_linearity", mode: 1, original_graph: null, trained_graph: null, selection: null });
    ...
    function changeSamples(e: any, v: any) {
        console.log(v);
        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: v, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
    }
    ...
    useEffect(() => {
        const mod = Module().then((prop: any) => {
            var zk = prop.generate_n(JSON.stringify(dormans), state.num_samples);
            var grphs = [];
...
* console.log(node); */
            ...
            zk.delete();
            setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: prop, graphs: grphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
        });
    }, []);
...
* console.log(Module); */
    ...
    function handleSelectT(name: any, value: any) {
        ...
        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: trained_graph, selection: state.selection });
    }
    function handleSelect(name: any, value: any) {
        var sk = state.graphs[value._vgsid_ - 1]
        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: sk, trained_graph: state.trained_graph, selection: state.selection });
    }

    function handleHover(name: any, value: any) {
        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: value });
    }
    function handleChange(e: any) {

        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: e.target.value, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });

    }
    function handleChangex(e: any) {

        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: e.target.value, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });

    }
    function handleChangey(e: any) {

        setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: e.target.value, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
    ...
    function handleTrain(e: any) {
        ...
        if (state.selection && state.module) {
            ...
            for (var i = 0; i < state.graphs.length; i++) {
                if (state.graphs[i][keyss[0]] > value[keyss[0]][0] && state.graphs[i][keyss[0]] < value[keyss[0]][1]) {
                    if (state.graphs[i][keyss[1]] > value[keyss[1]][0] && state.graphs[i][keyss[1]] < value[keyss[1]][1]) {
                        selected_graphs.push(state.graphs[i]);
                        vec.push_back(state.graphs[i].chain_str)
                    }
                }
            }
            var zk: any = state.module.generate_n_learnt_chains(JSON.stringify(dormans), state.num_samples, vec);
            vec.delete();
            var grphs = [];
            ...
            zk.delete();
            console.log(grphs);
            setState({ user_id: state.user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: grphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
            connectToDatabase(state.graphs, grphs, selected_graphs, state.user_id);
        }
    }

    function userIDChange(e: any) {
        setState({ user_id: state.user_id, temp_user_id: e.target.value, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
    }

    function submitUserID(e: any) {
        //TODO validate the submited id to make sure it is a valid ID
        //might have to grab that cookie
        if (!state.temp_user_id || !Number(state.temp_user_id) || state.temp_user_id.length !== 5) {
            console.log(state.temp_user_id.length)
            alert("Invalid user_id");
            return;

        }
        setState({ user_id: state.temp_user_id, temp_user_id: state.temp_user_id, num_samples: state.num_samples, module: state.module, graphs: state.graphs, trained_graphs: state.trained_graphs, x: state.x, y: state.y, mode: state.mode, original_graph: state.original_graph, trained_graph: state.trained_graph, selection: state.selection });
    }
    function makeUserID() {
        return <div> <h1> Please enter your Participant ID: </h1>
    ...
    function renderGraph(graph: any, msg: String) {
        ...
        return <div> <h2> {msg}</h2>
            <GraphComponent graph={graph} height={900} manipulate={false} />
        </div>
        ...
        if (state.user_id) {
            return <div className="App">
                ...
                <div className="graphs">
                    {renderGraph(state.original_graph, "Graph from Original Grammar")}
                    <div>
                        {renderGraph(state.trained_graph, "Graph from Trained Grammar")}
                    </div>
                </div>
            </div>
        } else {
